...
if __name__ == '__main__':
    ...
    for i in tqdm(range(n_runs)):
        ...
        qpw = np.dot(query_data, proto.transpose()).reshape(-1, n_shot, n_ways)
        qpw = F.softmax(torch.Tensor(qpw), dim=1)
        qpw = torch.unsqueeze(qpw.view(n_usamples,-1), 2).repeat(1, 1, query_data.shape[1])
        proto = torch.unsqueeze(torch.tensor(proto), 0).repeat(n_usamples, 1, 1)
        proto = torch.mul(proto, qpw)
        proto = torch.sum(proto.reshape(-1, n_shot, n_ways, query_data.shape[1]), axis=1)
        proto = proto / np.linalg.norm(proto, 2, 2)[:,:, None]

        query_data = torch.unsqueeze(torch.tensor(query_data), 2)
        query_data = query_data / np.linalg.norm(query_data, 2, 1)[:, None]
        predicts = torch.bmm( proto, query_data)
